...
from datasets.seq_halftinyimagenet import HalfTinyImagenet, SequentialHalfTinyImagenet, MyHalfTinyImagenet
...
class DreamDataset(MyHalfTinyImagenet):
    DREAM_CLASS_LABELS = []
    DREAM_CLASS_NAMES = []

    def __init__(self, root_1:str, root_2: str, train:bool = True, transform:transforms=None, target_transform=None, 
                 download:bool=False, labels=[x for x in range(SequentialHalfTinyImagenet.N_CLASSES_PER_TASK)] + [None] * SequentialHalfTinyImagenet.N_CLASSES_PER_TASK * (SequentialHalfTinyImagenet.N_TASKS-1)) -> None:
        super(DreamDataset, self).__init__(root_1, train, transform, target_transform, download)

        #aux data
        d2_name = os.path.split(root_2)[-1]
        if d2_name == 'TINYIMG':
            dataset_2 = HalfTinyImagenet(root_1, train, transform, target_transform, download, firstHalf=False)
        else:
        ...
        if len(targets_2) > 0:   
            if not data_1.shape[-3:] == data_2.shape[-3:]:
                ...
                tmp_data = []
                for img in data_2:
                    tmp_data.append(image_resize(img, dsize))                   
...
class DreamHalfTinyImagenet(SequentialHalfTinyImagenet):
    NAME = 'dream-seq-halftinyimg'
    ...
    def get_data_loaders(self):
        ...
        train_dataset = DreamDataset(base_path() + 'TINYIMG', base_path() + self.args.dataset_2,  train=True, download=True, transform=transform, labels = list(self.pos_to_label.values()))
        test_dataset = DreamDataset(base_path() + 'TINYIMG', base_path() + self.args.dataset_2, train=False, download=True, transform=test_transform, labels = list(self.pos_to_label.values()))
